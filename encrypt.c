...
void pad_enc (const unsigned char* m, unsigned char* mp, int l, int len8) {
  int i;
...
void reset_lfsr_56 (unsigned char* CNT) {
  CNT[0] = 0x01;
  CNT[1] = 0x00;
  CNT[2] = 0x00;
  CNT[3] = 0x00;
...
void nonce_encryption_ (const unsigned char* N,
		       unsigned char* CNT,
...
unsigned long long msg_encryption (const unsigned char** M, unsigned char** c,
  ...
  *M = *M + len8;
  lfsr_gf56(CNT);
  if (mlen != 0) {
    nonce_encryption_(N,CNT,s,k,t,n,D);
  }
  return mlen;
}
			 ...
			 )
{
  ...
  nonce_encryption_(N,CNT,s,k,t,n,w);
  ...
  *clen = mlen + n;
  ...
  if (mlen > 0) {
    nonce_encryption_(N,CNT,s,k,t,n,36);  
    while (mlen > n) {
